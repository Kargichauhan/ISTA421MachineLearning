...
def test_read_data(exercise_results):
    ...
    assert numpy.array_equal(X, target_X)
    assert numpy.array_equal(w, target_w)
...
def test_x_column_vectors(exercise_results):
    ...
    assert numpy.array_equal(x_n1, target_x_n1)
    assert numpy.array_equal(x_n2, target_x_n2)
...
def test_XX(exercise_results):
    ...
    targetXX = numpy.array([[1394., 756.],
                            [ 756., 563.]])
    assert numpy.array_equal(XX, targetXX)
